...
public class PlayerStatusManager : MonoBehaviour
{
    ...
    int verticalFromPlayerMovementManager;
    ...
    {// I didn't added the if not game paused condition because if game pauses, FixedUpdate pauses too.
        ...
        {
            flatVelocityMagnitude = new Vector2(playerRigidbody.velocity.x, playerRigidbody.velocity.z).magnitude;
            ...
            if (!PlayerMovementManager.crouching)
            {
                ...
                if (groundedForAllFromPlayerMovementManager)
                {
                    ...
                    jumpingUp = PlayerMovementManager.jumping && playerRigidbody.velocity.y > minimum;
                    goingUp = false;
                    goingDown = false;
                }
                else
                {
                    idling = false;
                    jumpingUp = false;
                    goingUp = playerRigidbody.velocity.y > minimum;
                    goingDown = playerRigidbody.velocity.y < -minimum;
                }
            ...
            else
            {
                ...
                if (groundedForAllFromPlayerMovementManager)
                {
                    ...
                    crouchJumpingUp = PlayerMovementManager.jumping && playerRigidbody.velocity.y > minimum;
                    crouchGoingUp = false;
                    crouchGoingDown = false;
                    sliding = flatVelocityMagnitude > PlayerMovementManager.normalMoveSpeed || PlayerMovementManager.onSlope;
                }
                else
                {
                    crouchIdling = false;
                    crouchJumpingUp = false;
                    crouchGoingUp = playerRigidbody.velocity.y > minimum;
                    crouchGoingDown = playerRigidbody.velocity.y < -minimum;
                }
        ...
        if (!runAndSlideParticles.isPlaying && groundedForAllFromPlayerMovementManager && !playerDiedFromPlayerSpawnAndSaveManager && flatVelocityMagnitude > PlayerMovementManager.runSpeed / 4 && (sliding || running))
        {
            runAndSlideParticles.Play();
        }
        else if (runAndSlideParticles.isPlaying && (!groundedForAllFromPlayerMovementManager || playerDiedFromPlayerSpawnAndSaveManager || flatVelocityMagnitude <= PlayerMovementManager.runSpeed / 4 || !(sliding || running)))
        {
